...
@php
    ...
    if ($initialItems === null) {
        $initialItems = $purchaseOrderModel?->items
            ?->map(fn ($item) => [
...
@section('content')
    <div class="space-y-6">
        ...
        <form method="POST" action="{{ $submitUrl }}" class="space-y-6" x-data="purchaseOrderForm(@js($productsForPicker), @js($initialItems))">
            @csrf
            ...
            <section class="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)]">
                    ...
                    <div class="rounded-[32px] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                        ...
                        <div class="mt-6 grid gap-5 md:grid-cols-2">
                            <label class="block">
                                ...
                                <select name="supplier_id" class="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-white">
                                    <option value="">Pilih supplier</option>
                                    @foreach ($suppliers as $supplier)
                                        <option value="{{ $supplier->id }}" @selected((string) old('supplier_id', $purchaseOrderModel?->supplier_id) === (string) $supplier->id)>{{ $supplier->name }}</option>
                                    @endforeach
                                </select>
                            </label>
                            ...
                            <label class="block">
                                ...
                                <select name="warehouse_id" class="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-white">
                                    ...
                                    @foreach ($warehouses as $warehouse)
                                        <option value="{{ $warehouse->id }}" @selected((string) old('warehouse_id', $purchaseOrderModel?->warehouse_id) === (string) $warehouse->id)>{{ $warehouse->name }}</option>
                                    @endforeach
                                </select>
                            </label>
                            ...
                            <label class="block">
                                ...
                                <input type="date" name="order_date" value="{{ old('order_date', $purchaseOrderModel?->order_date?->format('Y-m-d') ?? now()->format('Y-m-d')) }}" class="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-white">
                            </label>

                            <label class="block">
                                <span class="text-sm font-semibold text-gray-700 dark:text-gray-200">Expected Date</span>
                                <input type="date" name="expected_date" value="{{ old('expected_date', $purchaseOrderModel?->expected_date?->format('Y-m-d')) }}" class="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-white">
                            </label>

                            <label class="block md:col-span-2">
                                <span class="text-sm font-semibold text-gray-700 dark:text-gray-200">Terms</span>
                                <textarea name="terms" rows="3" class="mt-2 w-full rounded-3xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-white">{{ old('terms', $purchaseOrderModel?->terms) }}</textarea>
                            </label>

                            <label class="block md:col-span-2">
                                <span class="text-sm font-semibold text-gray-700 dark:text-gray-200">Catatan Buyer</span>
                                <textarea name="notes" rows="4" class="mt-2 w-full rounded-3xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-white">{{ old('notes', $purchaseOrderModel?->notes) }}</textarea>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="rounded-[32px] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                        <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                            <div>
                                ...
                                <h2 class="mt-4 text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">Daftar produk yang akan dibeli</h2>
                            </div>
                        ...
                        <div class="mt-6 space-y-4">
                            <template x-for="(item, index) in items" :key="index">
                                <div class="rounded-[28px] border border-gray-200 bg-gray-50/80 p-4 dark:border-gray-800 dark:bg-gray-900/70">
                                    <div class="grid gap-4 xl:grid-cols-[minmax(0,1.6fr)_repeat(3,minmax(0,0.7fr))_minmax(0,1fr)_auto]">
                                        <label class="block">
                                            <span class="text-xs font-semibold uppercase tracking-[0.22em] text-gray-400">Produk</span>
                                            <select :name="`items[${index}][product_id]`" x-model="item.product_id" @change="syncCost(index)" class="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-gray-400 dark:border-gray-800 dark:bg-gray-950 dark:text-white">
                                        ...
                                        <label class="block">
                                            ...
                                            <input type="number" min="0" step="0.01" :name="`items[${index}][ordered_quantity]`" x-model="item.ordered_quantity" class="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-gray-400 dark:border-gray-800 dark:bg-gray-950 dark:text-white">
                                        </label>

                                        <label class="block">
                                            <span class="text-xs font-semibold uppercase tracking-[0.22em] text-gray-400">Unit Cost</span>
                                            <input type="number" min="0" step="0.01" :name="`items[${index}][unit_cost]`" x-model="item.unit_cost" class="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-gray-400 dark:border-gray-800 dark:bg-gray-950 dark:text-white">
                                        </label>
                ...
                <div class="space-y-6">
                    <div class="rounded-[32px] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                        ...
                        <div class="mt-6 grid gap-3">
                            <div class="rounded-[24px] border border-gray-200 bg-gray-50/80 p-4 dark:border-gray-800 dark:bg-gray-900/70">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-gray-400">Jumlah item</p>
                                <p class="mt-2 text-2xl font-semibold text-gray-900 dark:text-white" x-text="items.length"></p>
                            </div>
                            <div class="rounded-[24px] border border-gray-200 bg-gray-50/80 p-4 dark:border-gray-800 dark:bg-gray-900/70">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-gray-400">Subtotal</p>
                                <p class="mt-2 text-2xl font-semibold text-gray-900 dark:text-white" x-text="money(total())"></p>
                            </div>
                            <div class="rounded-[24px] border border-gray-200 bg-gray-50/80 p-4 dark:border-gray-800 dark:bg-gray-900/70">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-gray-400">Workflow</p>
                                <p class="mt-2 text-sm leading-6 text-gray-500 dark:text-gray-400">Simpan sebagai draft jika buyer masih mengolah item. Gunakan submit untuk mengirim dokumen ke approval queue.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-[32px] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                        ...
                        <p class="mt-2 text-sm leading-6 text-gray-500 dark:text-gray-400">Setelah disubmit, purchase order akan masuk ke approval queue dan mulai memengaruhi KPI procurement di dashboard.</p>
...
@push('scripts')
    <script>
        function purchaseOrderForm(products, initialItems) {
            return {
                products,
                ...
                lineTotal(item) {
                    ...
                    return Math.max((quantity * cost) - discount, 0);
                },
                total() {
