        ...
        <section class="grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(360px,0.95fr)]">
                ...
                <div class="rounded-[32px] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="rounded-[24px] border border-gray-200 bg-gray-50/80 p-4 dark:border-gray-800 dark:bg-gray-900/70">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-gray-400">Invoice</p>
                            <p class="mt-2 text-xl font-semibold text-gray-900 dark:text-white">{{ $salesTransaction->invoice_number ?? $salesTransaction->transaction_number }}</p>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ $salesTransaction->invoice_date?->format('d M Y') ?? '-' }}</p>
                        </div>
                        <div class="rounded-[24px] border border-gray-200 bg-gray-50/80 p-4 dark:border-gray-800 dark:bg-gray-900/70">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-gray-400">Customer</p>
                            <p class="mt-2 text-xl font-semibold text-gray-900 dark:text-white">{{ $salesTransaction->customer?->name ?? ($salesTransaction->customer_name ?: 'Walk-in Customer') }}</p>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ $salesTransaction->outlet?->name ?? '-' }}</p>
                        </div>
                        <div class="rounded-[24px] border border-gray-200 bg-gray-50/80 p-4 dark:border-gray-800 dark:bg-gray-900/70">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-gray-400">Total Invoice</p>
                            <p class="mt-2 text-xl font-semibold text-gray-900 dark:text-white">Rp {{ number_format($salesTransaction->net_amount, 0, ',', '.') }}</p>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Due {{ $salesTransaction->due_date?->format('d M Y') ?? '-' }}</p>
                        </div>
                        <div class="rounded-[24px] border border-gray-200 bg-gray-50/80 p-4 dark:border-gray-800 dark:bg-gray-900/70">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-gray-400">Saldo</p>
                            <p class="mt-2 text-xl font-semibold text-gray-900 dark:text-white">Rp {{ number_format($salesTransaction->balance_due, 0, ',', '.') }}</p>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Paid Rp {{ number_format($salesTransaction->paid_amount, 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ $submitUrl }}" class="rounded-[32px] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <div class="grid gap-5 md:grid-cols-2">
                        ...
                        <label class="block">
                            ...
                            <input type="number" min="0" step="0.01" name="amount" value="{{ old('amount', $salesTransaction->balance_due) }}" class="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-white">
                        </label>
                        <label class="block">
                        ...
                        <label class="block md:col-span-2">
                            <span class="text-sm font-semibold text-gray-700 dark:text-gray-200">Approval Code</span>
                            <input type="text" name="approval_code" value="{{ old('approval_code') }}" class="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:text-white">
                        </label>
                    </div>
            ...
            <div class="rounded-[32px] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div class="mt-6 space-y-3">
                    @forelse ($salesTransaction->payments as $payment)
                        <div class="rounded-[24px] border border-gray-200 bg-gray-50/80 p-4 dark:border-gray-800 dark:bg-gray-900/70">
                            ...
                            <p class="mt-2 text-sm leading-6 text-gray-500 dark:text-gray-400">{{ $payment->settled_at?->format('d M Y H:i') ?? '-' }} / {{ $payment->reference_number ?: 'Tanpa referensi' }}</p>
                        </div>
                    @empty
                        <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">Belum ada pembayaran yang tercatat untuk invoice ini.</p>
                    @endforelse
                </div>
            </div>
        </section>
