                                ...
                                @foreach ($invoices as $invoice)
                                    <tr>
                                        <td class="px-5 py-4 align-top">
                                            ...
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $invoice['outlet'] }}</p>
                                        </td>
                                        <td class="px-5 py-4 align-top">
                                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $invoice['customer'] }}</p>
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $invoice['payments'] ?: 'Belum ada pembayaran' }}</p>
                                        </td>
                                        <td class="px-5 py-4 align-top">
                                            <p class="text-sm text-gray-700 dark:text-gray-300">{{ $invoice['invoice_date'] }}</p>
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Due {{ $invoice['due_date'] }}</p>
                                        </td>
                                        <td class="px-5 py-4 align-top">
                                            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $invoice['net_amount'] }}</p>
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Paid {{ $invoice['paid_amount'] }}</p>
                                        </td>
                                        <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $invoice['balance_due'] }}</td>
                                        <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">{{ $invoice['payment_status'] }}</td>
                                        <td class="px-5 py-4">
                                            @if ($invoice['can_pay'])
                                                <a href="{{ $invoice['payment_url'] }}" class="inline-flex items-center rounded-full border border-success-200 px-3 py-1.5 text-xs font-semibold text-success-700 transition hover:bg-success-50 dark:border-success-500/20 dark:text-success-300 dark:hover:bg-success-500/10">
                                                    Terima Pembayaran
                                                </a>
                                            @else
