    ...
    <div class="space-y-6">
        ...
        <section class="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(0,0.65fr)]">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                        <thead class="bg-gray-50 dark:bg-gray-900">
                            <tr>
                                ...
                                <th class="px-5 py-4 text-left text-xs font-semibold uppercase tracking-[0.24em] text-gray-400">Refund</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                            @forelse ($returns as $return)
                                <tr>
                                    ...
                                    <td class="px-5 py-4 align-top">
                                        <p class="text-sm text-gray-700 dark:text-gray-300">{{ $return['transaction_number'] }}</p>
                                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $return['invoice_number'] }} • {{ $return['outlet'] }}</p>
                                    </td>
                                    <td class="px-5 py-4 align-top">
                                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $return['refund_amount'] }}</p>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-5 py-6 text-sm text-gray-500 dark:text-gray-400">Belum ada retur penjualan.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="rounded-[28px] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                <p class="text-sm font-semibold text-gray-900 dark:text-white">Buat Retur Baru</p>
                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Pilih transaksi yang akan dibuatkan retur/refund.</p>
                <div class="mt-5 space-y-3">
                    @forelse ($createCandidates as $candidate)
                        <a href="{{ $candidate['create_url'] }}" class="block rounded-2xl border border-gray-200 bg-gray-50/80 p-4 transition hover:border-gray-300 dark:border-gray-800 dark:bg-gray-900/70">
                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $candidate['label'] }}</p>
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ $candidate['sold_at'] }}</p>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">{{ $candidate['net_amount'] }}</p>
                        </a>
                    @empty
                        <p class="text-sm text-gray-500 dark:text-gray-400">Belum ada transaksi kandidat retur.</p>
                    @endforelse
                </div>
            </div>
        </section>
