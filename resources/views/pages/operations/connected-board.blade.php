...
@section('content')
    <div class="space-y-6">
                        ...
                        @if (!empty($actions))
                            <div class="mt-6 flex flex-wrap gap-3">
                                @foreach ($actions as $action)
                                    <a href="{{ $action['url'] }}" class="inline-flex items-center rounded-full {{ ($action['variant'] ?? 'primary') === 'secondary' ? 'border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:border-gray-300 hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800' : 'bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-700 dark:bg-white dark:text-gray-900 dark:hover:bg-gray-200' }}">
                                        {{ $action['label'] }}
                                    </a>
                                @endforeach
                            </div>
        ...
        <section class="grid gap-6 {{ !empty($sideCards) ? '2xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]' : '' }}">
            <div class="rounded-[32px] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                <h2 class="text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ $mainTitle }}</h2>
                <p class="mt-3 max-w-2xl text-sm leading-6 text-gray-500 dark:text-gray-400">{{ $mainDescription }}</p>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                            <thead class="bg-gray-50 dark:bg-gray-900">
                                <tr>
                                    @foreach ($tableColumns as $column)
                                        <th class="px-5 py-4 text-left text-xs font-semibold uppercase tracking-[0.24em] text-gray-400">{{ $column }}</th>
                                    @endforeach
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-800 dark:bg-white/[0.03]">
                                @forelse ($tableRows as $row)
                                    <tr>
                                        @foreach ($tableColumns as $column)
                                            <td class="px-5 py-4 text-sm {{ $loop->first ? 'font-semibold text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-300' }}">{{ $row[$column] ?? '-' }}</td>
                                        @endforeach
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="{{ count($tableColumns) }}" class="px-5 py-6 text-sm text-gray-500 dark:text-gray-400">Belum ada data yang bisa ditampilkan untuk modul ini.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            @if (!empty($sideCards))
                <div class="rounded-[32px] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                    @if ($sideTitle)
                        <h2 class="text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ $sideTitle }}</h2>
                    @endif
